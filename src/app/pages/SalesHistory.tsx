...
import { 
  ...
  Hash, User, Package, Building2, RotateCcw, AlertTriangle, Check
} from 'lucide-react';
...
export function SalesHistory() {
  ...
  const [returnsPagination, setReturnsPagination] = useState({ page: 1, limit: 20, total: 0, totalPages: 1 });
  ...
  const [returnReason, setReturnReason] = useState<'PRODUCTO_DEFECTUOSO' | 'ERROR_EN_VENTA' | 'CAMBIO_DE_PRODUCTO' | 'INSATISFACCION_CLIENTE' | 'OTRO'>('PRODUCTO_DEFECTUOSO');
  const [returnNotes, setReturnNotes] = useState('');
  const [itemsToReturn, setItemsToReturn] = useState<Array<{ saleItemId: number, quantity: number, productName: string, maxQuantity: number, unitPrice: number, quantityPurchased: number }>>([]);
  ...
  }, [activeTab, pagination.page, statusFilter, paymentFilter, dateFilter, searchTerm]);
  ...
  }, [activeTab, returnsPagination.page, retStatusFilter, retSaleIdFilter, retStartDateFilter, retEndDateFilter]);
  ...
  const handleSearchSale = async () => {
    ...
    try {
      ...
      if (sale.status !== 'COMPLETADA') {
        toast.error('Solo se pueden registrar devoluciones de ventas en estado COMPLETADA');
        return;
      }
      ...
      history
        ...
        .forEach(r => {
          r.items.forEach(ri => {
            returned[ri.saleItemId] = (returned[ri.saleItemId] || 0) + Number(ri.quantity);
          });
        });
  ...
  const handleCreateReturnSubmit = async () => {
    ...
    if (itemsFiltered.length === 0) {
      toast.error('Debe seleccionar al menos un producto con cantidad a devolver mayor a 0');
      return;
    }
  ...
  const handleUpdateReturnStatus = async (newStatus: 'APROBADA' | 'RECHAZADA') => {
    if (!selectedReturn) return;
    if (newStatus === 'RECHAZADA' && !approvalNotes.trim()) {
      toast.error('Por favor especifique el motivo del rechazo en el campo de notas');
      return;
    }
    ...
    try {
      ...
      toast.success(`Solicitud de devolución ${newStatus === 'APROBADA' ? 'aprobada' : 'rechazada'} con éxito`);
      setDetailReturnModalOpen(false);
      fetchReturns();
    } catch (error: any) {
      toast.error(error.message || 'Error al actualizar el estado de la devolución');
    } finally {
  ...
  const getReturnStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDIENTE':
        return <Badge variant="outline" className="bg-amber-500/10 text-amber-500 border-amber-500/20 font-bold">Pendiente</Badge>;
      case 'APROBADA':
        return <Badge variant="outline" className="bg-blue-500/10 text-blue-500 border-blue-500/20 font-bold">Aprobada</Badge>;
      case 'COMPLETADA':
        return <Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 font-bold">Completada</Badge>;
      case 'RECHAZADA':
        return <Badge variant="outline" className="bg-rose-500/10 text-rose-500 border-rose-500/20 font-bold">Rechazada</Badge>;
      default:
  ...
  const handleInitiateReturn = async (sale: SaleResponse) => {
    ...
    try {
      ...
      toast.success(`Cargando ítems de la venta #${sale.id.toString().padStart(6, '0')}`);
    } catch (error: any) {
      toast.error(error.message || 'Error al buscar la venta');
      setCreateReturnModalOpen(false);
    } finally {
    ...
    <div className="flex flex-col gap-6 h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-sec)]">Consulta, anulación, reenvío de facturas y devoluciones de mercadería.</p>
        </div>
        {activeTab === 'history' && (
      ...
      {activeTab === 'history' ? (
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {sale.status === 'COMPLETADA' && (
                                <>
                                  ...
                                  >
                                    <RotateCcw size={14} className="mr-2 text-[var(--primary)]" /> Registrar Devolución
                                  </DropdownMenuItem>
                                  <DropdownMenuItem 
        ...
        <>
          <Card className="p-4 border-[var(--border)] bg-[var(--card)] shadow-sm">
            <div className="flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 w-full space-y-1.5">
                <Label className="text-xs font-bold text-[var(--text-sec)] uppercase">Estado Devolución</Label>
              ...
              <div className="flex-[1.2] w-full space-y-1.5">
                <Label className="text-xs font-bold text-[var(--text-sec)] uppercase">Venta Original (#)</Label>
                <div className="relative">
                  <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-sec)]" />
                  <Input 
                    type="number" 
              ...
              <div className="flex-1 w-full space-y-1.5">
                <Label className="text-xs font-bold text-[var(--text-sec)] uppercase">Desde</Label>
                <div className="relative">
                  <CalendarIcon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-sec)]" />
                  <Input 
                    type="date"
              ...
              <div className="flex-1 w-full space-y-1.5">
                <Label className="text-xs font-bold text-[var(--text-sec)] uppercase">Hasta</Label>
                <div className="relative">
                  <CalendarIcon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-sec)]" />
                  <Input 
                    type="date"
              ...
              <Button variant="outline" onClick={resetReturnsFilters} className="font-bold">
                <Filter size={16} className="mr-2" /> Limpiar
              </Button>
            </div>
          </Card>
          ...
          <div className="rounded-xl border overflow-hidden shadow-sm bg-[var(--card)] border-[var(--border)] flex-1 flex flex-col min-h-0">
            <div className="overflow-x-auto flex-1">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="font-bold w-[120px]">Devolución</TableHead>
                    <TableHead className="font-bold">Fecha</TableHead>
                    <TableHead className="font-bold">Venta Orig.</TableHead>
                    <TableHead className="font-bold">Cliente</TableHead>
                    <TableHead className="font-bold">Motivo</TableHead>
                    <TableHead className="font-bold text-right">Reintegro</TableHead>
                    <TableHead className="font-bold text-center">Estado</TableHead>
                    <TableHead className="font-bold text-center">Nota de Crédito</TableHead>
                    <TableHead className="font-bold text-right w-[80px]">Acciones</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {returnsLoading ? (
                    Array.from({ length: 5 }).map((_, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-right"><div className="h-4 bg-muted animate-pulse rounded w-16 ml-auto" /></TableCell>
                        <TableCell className="text-center"><div className="h-6 bg-muted animate-pulse rounded-full w-20 mx-auto" /></TableCell>
                        <TableCell className="text-center"><div className="h-4 bg-muted animate-pulse rounded w-24 mx-auto" /></TableCell>
                        <TableCell><div className="h-8 bg-muted animate-pulse rounded w-8 ml-auto" /></TableCell>
                      </TableRow>
                    ))
                  ) : returnsList.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={9} className="text-center py-12 text-[var(--text-sec)]">
                        No se encontraron devoluciones registradas con los filtros seleccionados.
                      </TableCell>
                    </TableRow>
                  ) : (
                    returnsList.map(ret => (
                      <TableRow key={ret.id} className="hover:bg-muted/10 transition-colors">
                        <TableCell className="font-bold text-[var(--primary)]">
                        ...
                        <TableCell className="text-center text-xs">
                          {ret.dteCreditNoteCode ? (
                            <div className="flex flex-col items-center">
                              ...
                              <span className="text-[9px] text-[var(--text-sec)] mt-0.5">DTE 05 Emitido</span>
                            </div>
                          ) : (
                        ...
                        <TableCell className="text-right">
                          <Button variant="ghost" size="icon" onClick={() => handleOpenReturnDetail(ret)} className="h-8 w-8 hover:bg-[var(--accent)] hover:text-[var(--primary)] cursor-pointer">
                            <Eye size={16} />
                          </Button>
                        </TableCell>
                      </TableRow>
            ...
            {returnsPagination.totalPages > 1 && (
              <div className="p-4 border-t border-[var(--border)] flex items-center justify-between bg-[var(--bg)]/5">
                <p className="text-xs font-bold text-[var(--text-sec)]">
                  Página {returnsPagination.page} de {returnsPagination.totalPages} ({returnsPagination.total} registros)
                </p>
                <div className="flex gap-2">
                  <Button 
                    variant="outline" size="sm" 
                    disabled={returnsPagination.page === 1}
                    onClick={() => setReturnsPagination(p => ({ ...p, page: p.page - 1 }))}
                  >
                    Anterior
                  </Button>
                  <Button 
                    variant="outline" size="sm" 
                    disabled={returnsPagination.page === returnsPagination.totalPages}
                    onClick={() => setReturnsPagination(p => ({ ...p, page: p.page + 1 }))}
                  >
                    Siguiente
                  </Button>
      ...
      <Dialog open={createReturnModalOpen} onOpenChange={(open) => { if(!open) { setCreateReturnModalOpen(false); resetCreateFlow(); } }}>
        <DialogContent 
        ...
        >
          ...
          <div className="p-6 border-b border-[var(--border)] flex items-start gap-4 bg-[var(--bg)]/50">
            ...
            <div>
               <h2 className="text-lg font-black text-[var(--text-main)] uppercase tracking-wide">Registrar Devolución</h2>
               <p className="text-sm font-mono text-[var(--text-sec)]">
                 {activeSale ? `Venta #${activeSale.id.toString().padStart(6, '0')}` : 'Buscar Venta'}
               </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  <div className="flex-1 space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)]">Buscar Venta por ID</Label>
                    <Input 
                      placeholder="Ej. 13" 
                      value={searchSaleId} 
                  ...
                  <div className="flex items-end">
                    <Button onClick={handleSearchSale} disabled={searchingSale || !searchSaleId} className="font-bold">
            ...
            {activeSale && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl border border-[var(--border)] bg-[var(--bg)]/30">
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Cliente</span>
                    <span className="font-bold text-[var(--text-main)] text-sm">{activeSale.customer?.name || 'Cliente General'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Fecha de Venta</span>
                    <span className="font-bold text-[var(--text-main)] text-sm">{new Date(activeSale.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Monto Total</span>
                    <span className="font-bold text-amber-500 text-sm">${Number(activeSale.totalAmount).toFixed(2)}</span>
                  </div>
                </div>

                {/* ITEMS TABLE */}
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)]">Productos a Devolver</Label>
                  <div className="rounded-xl border border-[var(--border)] overflow-hidden">
                    <Table>
                      <TableHeader className="bg-[var(--bg)]/50">
                        <TableRow>
                          <TableHead className="font-bold text-xs uppercase">Producto</TableHead>
                          <TableHead className="font-bold text-center text-xs uppercase">Comprado</TableHead>
                          <TableHead className="font-bold text-center text-xs uppercase">Devuelto</TableHead>
                          <TableHead className="font-bold text-right text-xs uppercase">Precio Unit.</TableHead>
                          <TableHead className="font-bold text-center w-32 text-xs uppercase">A Devolver</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {itemsToReturn.map((item, index) => {
                          ...
                          return (
                            <TableRow key={item.saleItemId}>
                              ...
                              <TableCell className="text-center text-sm font-semibold text-[var(--text-main)]">{item.quantityPurchased}</TableCell>
                              <TableCell className="text-center text-sm text-rose-500 font-semibold">{qtyDevuelta}</TableCell>
                              <TableCell className="text-right text-sm font-bold text-[var(--text-main)]">${Number(item.unitPrice).toFixed(2)}</TableCell>
                              <TableCell className="text-center">
                                <div className="flex items-center justify-center gap-2">
                                  ...
                                  <span className="w-8 text-center font-bold text-sm text-[var(--text-main)]">{item.quantity}</span>
                                  <Button 
                                    variant="outline" 
                                    size="icon" 
                                    type="button"
                                    ...
                                    onClick={() => {
                                      const newItems = [...itemsToReturn];
                                      newItems[index].quantity = Math.min(item.maxQuantity, newItems[index].quantity + 1);
                                      setItemsToReturn(newItems);
                                    }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)]">Motivo de Devolución</Label>
                    <Select value={returnReason} onValueChange={(v: any) => setReturnReason(v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="ERROR_EN_VENTA">Error en Venta / Cajero</SelectItem>
                        <SelectItem value="CAMBIO_DE_PRODUCTO">Cambio de Producto</SelectItem>
                        <SelectItem value="INSATISFACCION_CLIENTE">Insatisfacción del Cliente</SelectItem>
                        <SelectItem value="OTRO">Otro Motivo</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)]">Notas / Observaciones</Label>
                    <Input 
          ...
          <div className="p-6 border-t border-[var(--border)] bg-[var(--bg)]/30 flex justify-end gap-3">
            <Button 
              variant="outline" 
              onClick={() => { setCreateReturnModalOpen(false); resetCreateFlow(); }}
      ...
      <Dialog open={detailReturnModalOpen} onOpenChange={setDetailReturnModalOpen}>
        <DialogContent 
          className="max-w-4xl w-[95vw] p-0 overflow-hidden bg-[var(--card)] border-[var(--border)]"
          style={{ maxWidth: '900px', width: '90vw' }}
        >
          ...
          <div className="p-6 border-b border-[var(--border)] flex items-start gap-4 bg-[var(--bg)]/50">
            ...
            <div className="flex-1">
               ...
               <p className="text-sm font-mono text-[var(--text-sec)]">
                 {selectedReturn ? `DEV-${selectedReturn.id.toString().padStart(6, '0')}` : ''}
               </p>
            </div>
          </div>
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Información General</span>
                  <div className="text-xs space-y-1">
                    <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Venta Origen:</span> Venta #{selectedReturn.saleId.toString().padStart(6, '0')}</p>
                    <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Fecha Solicitud:</span> {new Date(selectedReturn.createdAt).toLocaleDateString()} {new Date(selectedReturn.createdAt).toLocaleTimeString()}</p>
                    <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Cajero:</span> {selectedReturn.user?.fullName || 'Desconocido'}</p>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Cliente</span>
                  <div className="text-xs space-y-1">
                    <p className="font-bold text-sm text-[var(--text-main)]">{selectedReturn.sale?.customer?.name || 'Consumidor Final'}</p>
                    {selectedReturn.sale?.customer?.nit && <p className="font-mono text-[var(--text-sec)]">NIT: {selectedReturn.sale.customer.nit}</p>}
                    {selectedReturn.sale?.customer?.documentNumber && <p className="font-mono text-[var(--text-sec)]">DUI/DOC: {selectedReturn.sale.customer.documentNumber}</p>}
                  </div>
                </div>

                {/* MOTIVO & OBSERVACIONES */}
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Motivo & Observaciones</span>
                  <div className="text-xs space-y-1">
                    <p className="font-semibold text-amber-600">{getReasonBadge(selectedReturn.reason)}</p>
                    <p className="text-[var(--text-sec)] italic">"{selectedReturn.notes || 'Sin observaciones'}"</p>
                  </div>
                </div>
              </div>
              ...
              {/* ITEMS TABLE */}
              <div className="space-y-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)]">Artículos a Devolver</span>
                <div className="rounded-xl border border-[var(--border)] overflow-hidden">
                  <Table>
                    <TableHeader className="bg-[var(--bg)]/50">
                      <TableRow>
                        ...
                        <TableHead className="font-bold text-right">Total Devolución</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {selectedReturn.items.map((item) => (
                        <TableRow key={item.id}>
                          <TableCell className="font-bold text-sm text-[var(--text-main)]">{item.product?.name || 'Producto Desconocido'}</TableCell>
                          <TableCell className="text-center text-sm font-semibold text-[var(--text-main)]">{item.quantity}</TableCell>
                          <TableCell className="text-right text-sm font-semibold text-[var(--text-main)]">${Number(item.unitPrice).toFixed(2)}</TableCell>
                          <TableCell className="text-right text-sm font-black text-emerald-600">${Number(item.subtotal).toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                  <div className="p-4 border-t border-[var(--border)] bg-[var(--bg)]/50 flex justify-end">
                    <div className="flex items-center gap-8">
                      <span className="text-xs font-bold text-[var(--text-sec)] uppercase tracking-widest">Total Reintegro</span>
                      <span className="text-xl font-black text-emerald-600">${Number(selectedReturn.totalRefund).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {selectedReturn.dteCreditNoteCode && (
                <div className="p-4 rounded-xl border border-indigo-500/20 bg-indigo-500/5 space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-indigo-500 block">Nota de Crédito Electrónica (DTE 05)</span>
                  <div className="text-xs space-y-1">
                    <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Código de Generación:</span> <span className="font-mono text-indigo-600 font-bold">{selectedReturn.dteCreditNoteCode}</span></p>
                    {selectedReturn.dteCreditNoteSello && (
                      <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Sello de Recepción:</span> <span className="font-mono text-indigo-600 font-bold">{selectedReturn.dteCreditNoteSello}</span></p>
                    )}
                    <p className="font-semibold text-[var(--text-main)]"><span className="text-[var(--text-sec)] font-normal">Estado en Hacienda:</span> <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-200">{selectedReturn.dteCreditNoteEstado || 'PROCESADO'}</Badge></p>
                  </div>
                </div>
              )}
              ...
              {selectedReturn.status === 'PENDIENTE' && canApprove && (
                <div className="p-6 rounded-xl border border-[var(--border)] bg-[var(--bg)]/40 space-y-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-sec)] block">Panel de Evaluación de Supervisor</span>
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label className="text-xs font-bold text-[var(--text-sec)]">Notas del Evaluador (Requerido para rechazo)</Label>
                      <Input 
                    ...
                    {selectedReturn.sale?.dteResponse?.codigoGeneracion && (
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <Label htmlFor="emit-credit-note-chk" className="text-xs font-semibold cursor-pointer text-[var(--text-main)]">
                          Emitir Nota de Crédito Electrónica en Hacienda (DTE 05) de forma automática al aprobar
                        </Label>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      ...
                      >
                        {submittingStatus ? 'PROCESANDO...' : 'APROBAR Y LIQUIDAR'}
                      </Button>
                    </div>
                  </div>
                </div>
              ...
              {selectedReturn.status !== 'PENDIENTE' && (
                <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--bg)]/10 space-y-1">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-sec)] block">Detalle de Resolución</span>
                  <p className="text-xs font-semibold text-[var(--text-main)]">
                    <span className="text-[var(--text-sec)] font-normal">Estado Final:</span> <span className={selectedReturn.status === 'APROBADA' || selectedReturn.status === 'COMPLETADA' ? 'text-emerald-600 font-bold' : 'text-rose-600 font-bold'}>{selectedReturn.status}</span>
                  </p>
                  {selectedReturn.notes && (
                    <p className="text-xs font-semibold text-[var(--text-main)]">
                      <span className="text-[var(--text-sec)] font-normal">Comentarios de Supervisor:</span> "{selectedReturn.notes}"
                    </p>
                  )}
                </div>
              )}
